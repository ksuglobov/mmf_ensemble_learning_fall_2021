...
from html_forms import NewEnsembleForm, HyperParamForm
from html_forms import UploadForm, LearnForm, TestForm
from main_classes import Ensemble, Dataset

app = Flask(__name__)
app.config['SECRET_KEY'] = 'stuntmanmike'
...
def get_models():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('settings', name=form.name.data))
...
def settings(name):
    ...
    return render_template('settings.html', **template_kwargs)
...
def model_page(name):
    """
    when model is ready for fitting
    """
    ...
    test_form.test_data.choices = list(datasets)
